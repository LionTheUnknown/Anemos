...
const RainEffect = (props) => {
  const {type, backgroundImageUrl, fillContainer = false } = props;
  ...
  const setBackgroundImage = (backgroundImageUrl) => {
    ...
    // Skip crossOrigin for data URLs - can cause load failures
    ...
    backgroundImage.onload = () => {
      // 1x1 SVG often reports 0x0; enforce minimum for WebGL
      const minSize = 256;
      const w = Math.max(backgroundImage.naturalWidth || 1, 1);
      const h = Math.max(backgroundImage.naturalHeight || 1, 1);
      textureFgSize = { width: Math.max(w, minSize), height: Math.max(h, minSize) };
      textureBgSize = { width: textureFgSize.width, height: textureFgSize.height };
    ...
    backgroundImage.onerror = () => {
      // Fallback: use minimal size and init anyway
      textureFgSize = { width: 256, height: 256 };
      textureBgSize = { width: 256, height: 256 };
      loadTextures().then(() => init(type));
    };
  }
  ...
  const setupLightningFlicker = () => {
    ...
    return (
      <div className="rain-effect-container" style={{width: '100%', height: fillContainer ? '100%' : '100vh'}}>
