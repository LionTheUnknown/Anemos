...
function SnowParticles({ settings }) {
  ...
  React.useEffect(() => {
    ...
    loader.load(typeof snowflake === 'string' ? snowflake : snowflake.src, (tex) => {
      setTexture(tex);
    });
  }, []);
...
export default function SnowEffect({ type = 'gentle', backgroundImageUrl, fillContainer = false }) {
  const settings = type === 'storm' ? STORM_SETTINGS : GENTLE_SETTINGS;
  return (
    <div style={{ width: fillContainer ? "100%" : "100vw", height: fillContainer ? "100%" : "100vh", backgroundImage: backgroundImageUrl ? `url(${backgroundImageUrl})` : 'none', backgroundSize: "cover", backgroundPosition: "center" }}>
